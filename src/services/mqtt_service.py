...
class MQTTService:
    ...
    def __init__(self, socketio: SocketIO, app=None):
        ...
        # Mapeamento de tipos de sensores (apenas para nome amigável inicial)
        self.sensor_types = {
    ...
    def _on_message(self, client, userdata, msg):
        ...
        try:
            ...
            gateway_id = topic_parts[1]  # CABR-A09D5843CA48 (não usado diretamente no DB, mas pode ser para logs)
    ...
    def _process_sensor_data(self, gateway_id: str, sensor_uid: str, payload: dict):
        ...
        try:
            # Buscar container (sensor) existente
            container = Container.query.get(sensor_uid) # uid é a PK agora
            
            if not container:
                # Se o sensor não existe, criá-lo com dados padrão
                print(f"⚠️ Sensor {sensor_uid} não encontrado. Criando novo sensor.")
                sensor_name = self.sensor_types.get(sensor_uid, f"Sensor {sensor_uid}")
                container = Container(
                    uid=sensor_uid,
                    nome_amigavel=sensor_name,
                    coordenada_x=-46.6333,  # Coordenadas padrão (São Paulo)
                    coordenada_y=-23.5505,
                    ativa=True,
                    nivel=0 # Nível inicial
                )
                db.session.add(container)
                db.session.commit() # Commit para que o container tenha um UID persistido
                print(f"✅ Novo sensor {sensor_uid} criado.")
            
            # Calcular nível de preenchimento baseado na distância
            # Distâncias maiores = container mais vazio
            # Assumindo altura do container de 100cm (pode ser ajustado)
            ...
            # Criar registro de dados do sensor na tabela mensagens_sensores
            sensor_data = SensorData(
                uid_sensor=sensor_uid,
                nSeq=payload['nSeq'],
                rssi=payload['rssi'],
                battery_pct=payload['battery_pct'],
                dist=payload['dist'],
                timestamp=payload['timestamp'],
                fill_level=fill_level, # Campo adicional para compatibilidade
                received_at=datetime.utcnow()
            )
            
            db.session.add(sensor_data)
            
            # Atualizar status do container (tabela sensores)
            container.nivel = int(fill_level) # Nível de preenchimento
            container.ativa = True # Assumir que está ativo se enviando dados
            container.last_updated = datetime.utcnow() # Campo adicional para compatibilidade
            ...
            self.socketio.emit('sensor_data', {
                ...
                'gateway_id': gateway_id, # Manter para compatibilidade com frontend
                'fill_level': fill_level,
                'battery_pct': payload['battery_pct'],
                'rssi': payload['rssi'],
                'timestamp': payload['timestamp'],
                'sensor_type': container.nome_amigavel # Usar nome amigável do DB
            })
    ...
    def _check_alerts(self, container: Container, sensor_data: SensorData):
        ...
        try:
            ...
            if container.nivel >= 90:
                alert = Alert(
                    type='container_full',
                    title='Container Cheio',
                    message=f'Container {container.nome_amigavel} está {container.nivel}% cheio',
                    container_uid=container.uid,
                    severity='high',
                    data={'fill_level': container.nivel}
                )
            ...
            if sensor_data.battery_pct is not None and sensor_data.battery_pct <= 20:
                alert = Alert(
                    type='low_battery',
                    title='Bateria Baixa',
                    message=f'Sensor {container.nome_amigavel} com bateria em {sensor_data.battery_pct}%',
                    container_uid=container.uid,
                    severity='medium',
            ...
            if sensor_data.rssi is not None and sensor_data.rssi <= -80:
                alert = Alert(
                    type='weak_signal',
                    title='Sinal Fraco',
                    message=f'Sensor {container.nome_amigavel} com sinal fraco (RSSI: {sensor_data.rssi})',
                    container_uid=container.uid,
                    severity='low',
